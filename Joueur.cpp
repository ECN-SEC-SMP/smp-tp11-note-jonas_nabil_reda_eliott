...
int Joueur::getNbTicketsReussis() const { return nbticketsreussis; }
const std::vector<CarteTrain*>& Joueur::getCartes() const { return Main_carte; }
std::vector<Ticket>& Joueur::getTickets() { return mesTickets; }

// Methodes
void Joueur::addCarte(CarteTrain* carte) { Main_carte.push_back(carte); }
void Joueur::addTicket(Ticket t) { mesTickets.push_back(t); }
void Joueur::addWagons(int nb) { nbwagon += nb; }
...
void Joueur::incrementerScore(int points) { score += points; }
void Joueur::incrementerTicketsReussis() { nbticketsreussis++; }
